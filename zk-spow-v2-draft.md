...
**Theorem 1 (Computational Progress-Freedom).** *Under the PRP assumption on Width-24 Poseidon2, the PoW success events $E_1, \ldots, E_P$ across all Merkle tree evaluations are computationally indistinguishable from i.i.d. Bernoulli($q$), with $q = 1 - (1-T/p^8)^3$, even given arbitrary auxiliary information $\mathrm{aux}$ derived from computation state (Merkle level, STARK phase progress, header update history).*

*Proof sketch.* Merkle tree inputs—full 24-element vectors $(n_L, n_R, h_H) \in \mathbb{F}_p^{24}$—are pairwise distinct except with negligible probability ($< 10^{-60}$ for $P \leq 10^7$, by union bound on PRP output collisions). For distinct inputs, the PRP assumption guarantees that $(\pi(x_1), \ldots, \pi(x_P))$ is computationally indistinguishable from independent uniform outputs. Each ticket comparison $\text{ticket} < T$ is a deterministic function of the output, so the derived events inherit independence. The PRP–PRF switching cost is $P^2/(2p^{24}) < 10^{-210}$, negligible. ∎
...
| $d_{reg}$ | Complexity | Interpretation |
|---|---|---|
...
| 14 | $\sim 2^{150}$ | Requires structural advance beyond [12] |
| $\leq 11$ | $\leq 2^{124}$ | Below 128-bit security threshold |

Reaching the security threshold ($d_{reg} \leq 11$) requires reducing the effective degree of regularity to less than 40% of the extrapolated value—equivalently, an attack must neutralize the algebraic contribution of the majority of the 30 rounds. The degree of regularity is determined by the round structure (S-box degree growth and MDS mixing); full state visibility does not change the round structure but does expose more equations to the attacker. Width-24's larger state introduces more intermediate variables than narrower configurations, increasing Gröbner basis dimension, though [12] shows that operation mode can affect practical attack complexity independently of $d_{reg}$. Note: $d_{reg} = 28$ is extrapolated from small-instance experiments on narrower Poseidon variants; the exact value for Width-24 over M31 has not been computed (§9).
