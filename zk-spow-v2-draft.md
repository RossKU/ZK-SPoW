...
Proof-of-work (PoW) blockchains expend energy solely for network security. A fundamental requirement—often implicit—is that PoW mining must be *progress-free*: each trial must succeed or fail independently, yielding memoryless (Poisson) block discovery. This property underpins fairness, difficulty adjustment, and the security proofs of DAG protocols such as DAGKnight.

Proof of Useful Work (PoUW) attempts to reclaim mining energy for productive computation—a direction Ball et al. [1] show faces fundamental deployment constraints. Recent proposals use ZK proof generation as the useful work, but face a tension: STARK proof generation is inherently *non-memoryless* (stateful, multi-phase, minutes to complete). If proof completion is the lottery event, miners who have invested more partial work have a progress advantage, violating the memoryless requirement.

**ZK-SPoW** (ZK-Symbiotic Proof of Work) resolves this tension by operating PoW at the *permutation level*, not the proof level. Every Poseidon2 permutation within the STARK Merkle tree simultaneously advances the ZK proof and produces PoW tickets. Under the pseudorandom permutation (PRP) assumption, each permutation output is independent of all others—regardless of the structured, stateful Merkle tree context—yielding Bernoulli trials and Poisson block discovery. Maximum header staleness is bounded by one Merkle commitment phase (~3 ms on GPU, <1 ms on ASIC), well within DAGKnight's tolerance at 100 blocks per second.

We instantiate ZK-SPoW for Kaspa, replacing kHeavyHash with Width-24 Poseidon2 over the Mersenne field $\mathbb{F}_p$, $p = 2^{31}-1$ (M31), achieving per-permutation usefulness $U = 16/24 \approx 67\%$ with three PoW tickets per permutation. PoW mining (compute-bound) and STARK proving (memory-bound) share the same hardware with zero switching overhead. Security claims assume final Poseidon2 production parameters (§9).
