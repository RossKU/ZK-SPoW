...
\item \textbf{GPU empirical validation.} We validate the two core claims---free PoW tickets from STARK computation and hashrate invariance across operating modes---on commodity GPU hardware, confirming the construction operates as specified before ASIC fabrication (Appendix~\ref{app:gpu}).
\end{enumerate}
...
Every Poseidon2 invocation in the STARK computation simultaneously: \textbf{(a)}~advances the ZK proof (economic value---the useful work that drives the permutation), and \textbf{(b)}~produces PoW tickets (network security---mathematical byproduct of the same permutation). GPU validation confirms up to 144.54M free tickets/s from STARK proof generation at zero additional cost (Appendix~\ref{app:gpu}, Table~\ref{tab:free-tickets}).
...
\item \textbf{Complementary bottleneck validation (partially resolved on GPU).} GPU validation confirms input-source independence (Random vs Merkle ratio 101.9\%, Appendix~\ref{app:gpu}) and free ticket generation from STARK computation (up to 144.54M tickets/s, Table~\ref{tab:free-tickets}). Full validation on actual ASIC designs remains pending.
...
We validate two core claims on commodity GPU hardware (CUDA implementation). These tests confirm that the ZK-SPoW construction operates as specified on real hardware before ASIC fabrication.

\subsection{Free Ticket Throughput}
\label{sec:gpu-freetickets}

\textbf{Claim (\S\ref{sec:symbiotic}).} ZK-SPoW's Width-24 Poseidon2 STARK prover generates PoW tickets from all Merkle hashes as a byproduct of standard ZK proof computation, at zero additional cost. Width-16 Stwo produces 0~tickets from the same computation.
...
\textbf{Interpretation.} Poseidon2 computation is input-independent (confirmed in \S\ref{sec:gpu-invariance}: ratio 101.9\%). Whether $S[16..23]$ carries $\mathit{header\_digest}$ or zeros, the same 30-round permutation executes. STARK proof time does not change with or without header binding. Pure ZK (Width-16) structurally lacks header binding and produces 0~PoW tickets. ZK-SPoW converts all Merkle hashes into PoW tickets at zero marginal cost.
...
\textbf{Claim (\S\ref{sec:modes}).} Poseidon2 permutation throughput is independent of the input source---random nonces (PoW mode) vs deterministic Merkle data (STARK mode).
...
\midrule
1 & 268.03 & 286.19 & 106.8\% \\
2 & 276.27 & 281.30 & 101.8\% \\
3 & 277.32 & 279.97 & 101.0\% \\
4 & 276.54 & 278.63 & 100.8\% \\
5 & 271.93 & 270.99 & 99.7\% \\
6 & 261.50 & 270.17 & 103.3\% \\
7 & 261.85 & 271.48 & 103.7\% \\
8 & 268.92 & 267.16 & 99.3\% \\
9 & 274.78 & 277.04 & 100.8\% \\
10 & 271.87 & 275.71 & 101.4\% \\
\midrule
\textbf{Mean} & \textbf{270.90} & \textbf{275.86} & \textbf{101.9\%} \\
\bottomrule
\end{tabular}
\caption{Hashrate invariance: Random (PoW) vs Merkle (STARK) input, 10 runs.}
\label{tab:invariance}
\end{table}

\textbf{Interpretation.} The mean ratio is 101.9\%, with Runs~5 and~8 showing inversions (ratio $<100\%$). The ${\sim}2\%$ variation falls within GPU thermal throttling and clock frequency fluctuation. Poseidon2 throughput is confirmed to be independent of input source, validating the hashrate invariance claim (\S\ref{sec:modes}): switching between Symbiotic and Pure PoW modes incurs zero throughput penalty.
