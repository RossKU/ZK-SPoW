...
B: Width 20 & 20 & Compression & 1 & 2 (+4 wasted) & +22\% & ${\sim}$+11\% \\
...
\textbf{Design~A:} No Poseidon2 modification. Standard width-16 sponge with 3~absorptions (left, right, header). 3~permutations per Merkle hash. PoW output is available only after the final permutation, yielding 1~ticket per 3~permutations---a $6\times$ lower PoW rate per permutation than Design~C. STARK throughput per permutation is $1/3$ that of compression function mode (1~hash per 3~perm vs 1~hash per 1~perm), though under SRAM-bandwidth-bound operation (\S\ref{sec:bottleneck}) the bottleneck is memory, not compute.

\textbf{Design~B:} Extend to width~20 with $\mathit{header\_digest}[4]$. Compression function mode, 1~permutation per hash, 2~PoW tickets ($S[0..7]$, $S[8..15]$). However, header digest is only 4~M31 elements (124 bits), giving a birthday collision bound of $2^{62}$---well below the 128-bit security target. Asymmetric $8{+}8{+}4$ I/O with 4~wasted output elements.
