...
\textbf{Preimage resistance (PoW security).} PoW requires only preimage resistance of the 248-bit output. The 30-round Poseidon2 permutation (8 external + 22 internal) satisfies this: no known attack reduces the preimage search below the generic $2^{248}$ bound, and the round count follows Plonky3's security formula with margin~\cite{plonky3}.
...
\textbf{Recent cryptanalysis.} Merz and Rodr{\'\i}guez Garc{\'\i}a~\cite{skipping2026} improve algebraic CICO attacks on Poseidon2 by exploiting $M_I$'s sparse structure (round-skipping). For one recommended 128-bit parameter set, the improvement is $2^{106}$ over prior art; however, the authors note that the full-round primitive ``does not fall short of its claimed security level.'' Separately, resultant-based attacks~\cite{resultant2026} solve small-scale Poseidon2-M31 bounty instances ($R_f \leq 6$, $R_p \leq 4$; $\leq$10 total rounds). These results do not affect the 30-round ($8{+}22$) configuration: the security margin ($R_f{+}{=}2$, $R_p \times 1.075$) absorbs the improved attack complexities.
